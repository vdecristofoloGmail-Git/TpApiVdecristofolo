...
module.exports = async function (req, res, next) {
    ...
    if (req.header('Authorization') && req.header('Authorization').split(' ').length > 1) {
        try {
            ...
            if (!cliente) return next(errors.ClienteNoAutorizado)
            
            res.locals.cliente = cliente //me puedo guardar el usuario en el locals para usarlo en las rutas que necesiten el usuario
        ...
        return next(errors.ClienteNoAutorizado)
    }
